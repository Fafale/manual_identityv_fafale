...
# Function to check options on the YAML and disable corresponding categories
def check_yaml_category(multiworld: MultiWorld, player: int, category_name: str) -> Optional[bool]:
    if category_name.startswith("CATEGORY_"):
        if (category_name == "CATEGORY_BT"): return (is_option_enabled(multiworld, player, "shuffle_borrowed_time"))
        elif (category_name == "CATEGORY_UNIQUECHALLENGE"): return (is_option_enabled(multiworld, player, "enable_unique_challenges"))
        elif (category_name == "CATEGORY_CLASSCHALLENGE"): return (is_option_enabled(multiworld, player, "enable_class_challenges"))
    
    return None

def check_disabled_character_category(multiworld: MultiWorld, player: int, category_name: str) -> Optional[bool]:
...
def is_category_enabled(multiworld: MultiWorld, player: int, category_name: str) -> bool:
    ...
    ret = check_yaml_category(multiworld, player, category_name)
    if ret is not None: hook_result = ret
